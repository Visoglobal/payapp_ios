...
- (void)setImageWithURL:(NSURL *)url
{
    [self setImageWithURL:url placeholderImage:nil options:0 progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType) {
                    if(self && (!image || error)){
                        UIImage* img = [UIImage imageNamed:@"loadingPreloader.png"];
                        self.image = img;
                    }
               }];
}
...
//        LoadingIndicator* loadingview = [[LoadingIndicator alloc] initWithFrame:CGRectMake(0, 0, 37, 37)];
//        __weak UIImageView *wself = self;
//        wself.animationImages = loadingview.ringImage.animationImages;
//        wself.animationDuration = loadingview.ringImage.animationDuration;
//        wself.contentMode = loadingview.ringImage.contentMode;
