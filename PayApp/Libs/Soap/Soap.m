...
+(id)objectFromNode:(CXMLNode*)node {
	...
	if([children count] > 0) {
		...
		for (CXMLNode* child in children) {
			...
			if(value != nil) {
				if([value isKindOfClass:[NSMutableArray class]] == NO) {
                    ...
                    [dictionary setObject:value forKey:[child name]];//my array fix
				}
				if(childValue != nil) {
		...
		if ([[dictionary allKeys] count] == 1 &&
            [[[dictionary allKeys] objectAtIndex:0] isKindOfClass:[NSString class]] &&
            [[[dictionary allKeys] objectAtIndex:0] isEqualToString:@"text"])
        {
